...
> **当前版本：测试版（CityUpedia测试版）**
> 本分支（`v1.0`）为**测试版**，所有功能开发与测试都在这里进行；验证通过后再同步到正式版（`main` 分支）。正式版沿用 `CityU Pedia` 品牌名。测试版对照城大官网核对了计算学院七大授课型硕士项目的核心课程，补全生物统计（MSBIOS）2 门核心课（PH5102/PH5103）与电子商贸（MSEC）信息安全核心课映射，并完成全站数据校验与 HTTP 加载测试，课程 PDF 覆盖率达 100%。
